...
from game_agent.core.decision import DecisionModule, Action, ActionType, ActionPriority
from game_agent.core.memory import AgentMemory
from game_agent.core.perception import PerceptionData, EntityInfo
...
class TestDecisionModule:
    ...
    def test_idle_decision(self):
        ...
        actions = self.decision.decide(perception, self.memory, current_state="idle")
        assert len(actions) > 0
        assert actions[0].action_type in (ActionType.PATROL, ActionType.IDLE)

    def test_threat_response(self):
        """发现威胁应生成攻击行动"""
        threat = EntityInfo(entity_id="enemy_01", entity_type="enemy", state="hostile", distance=10)
        perception = PerceptionData(threats=[threat])
        actions = self.decision.decide(perception, self.memory, current_state="alert")
        assert any(a.action_type == ActionType.ATTACK for a in actions)

    def test_low_health_flee(self):
        """低生命值应生成逃跑行动"""
        threat = EntityInfo(entity_id="enemy_01", entity_type="enemy", state="hostile", distance=5)
        perception = PerceptionData(threats=[threat])
        actions = self.decision.decide(
            perception, self.memory,
    ...
    def test_player_interaction(self):
        """附近有玩家应触发交互"""
        player = EntityInfo(entity_id="player_01", entity_type="player", distance=5)
        perception = PerceptionData(visible_players=[player])
        actions = self.decision.decide(perception, self.memory, current_state="patrol")
