...
class TestAgentMemory:
    ...
    def test_store_short_term(self):
        entry = self.memory.store({"event": "saw_player"}, MemoryType.SHORT_TERM)
    ...
    def test_auto_promote_high_importance(self):
        self.memory.store({"event": "critical"}, MemoryType.SHORT_TERM, importance=0.9)
        # 高重要性自动提升
        assert self.memory.stats["long_term"] >= 1

    def test_retrieve(self):
        self.memory.store({"type": "combat", "enemy": "wolf"}, MemoryType.SHORT_TERM, tags=["combat"])
        self.memory.store({"type": "social", "npc": "merchant"}, MemoryType.SHORT_TERM, tags=["social"])
    ...
    def test_consolidate(self):
        entry = self.memory.store({"a": 1}, MemoryType.SHORT_TERM, importance=0.8)
