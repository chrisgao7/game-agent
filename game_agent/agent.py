...
from game_agent.core.perception import PerceptionModule, PerceptionData
from game_agent.core.decision import DecisionModule, Action
...
class GameAgent:
    ...
    def decide(self, perception: PerceptionData) -> list[Action]:
        ...
        actions = self.decision_maker.decide(
            ...
            agent_properties={"health": self.health, "role": self.role, **self.properties},
        )
        return actions
    ...
    def execute(self, actions: list[Action]) -> list[ActionResult]:
        ...
        for action in actions:
            result = self.action_executor.execute(action, self.agent_id, self.game_world)
