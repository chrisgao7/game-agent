...
class PlayerProfile:
    ...
    completed_quests: dict[str, int] = field(
        default_factory=dict)  # type -> count
...
class PersonalizedContentGenerator:
    ...
    def generate_dialogue(
        ...
        if not templates:
            templates = self._dialogue_templates.get(
                'default', {}).get(rel_key, ['...'])
    ...
    def add_quest_template(self, template: QuestTemplate):
        self._quest_templates.setdefault(
            template.quest_type, []).append(template)

    def add_dialogue_templates(self, npc_role: str, relationship: str, templates: list[str]):
        self._dialogue_templates.setdefault(npc_role, {}).setdefault(relationship, []).extend(
            templates
        )
    ...
    def _determine_preferred_quest_type(self, player: PlayerProfile) -> str:
        ...
        weights = style_weights.get(
            player.play_style, style_weights['balanced'])
    ...
    def _generate_quest_parameters(
        ...
        """根据玩家等级和模板生成任务参数"""
        level = player.level
        difficulty = max(template.difficulty_range[0], min(
            template.difficulty_range[1], level))
