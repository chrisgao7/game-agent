...
class DynamicDifficultyAdjustment:
    ...
    def __init__(
        ...
        self._action_history: deque[PlayerAction] = deque(maxlen=evaluation_window * 5)
    ...
    def record(
        ...
        """便捷记录接口"""
        self.record_player_action(PlayerAction(
    ...
    def calculate_performance_score(self) -> float:
        ...
        efficiency = max(0, min(1, 1 - (avg_duration - expected_duration) / expected_duration)) if expected_duration > 0 else 0.5

        # 存活能力 (低伤害 = 高分)
        avg_damage = sum(a.damage_taken for a in recent) / len(recent) if recent else 0
        survival = max(0, min(1, 1 - avg_damage))

        # 资源效率 (低消耗 = 高分)
        avg_resources = sum(a.resources_used for a in recent) / len(recent) if recent else 0
    ...
    def adjust_difficulty(self) -> DifficultyParams:
        ...
        delta = (performance - self.target_success_rate) * self.adjustment_speed
